...
proc GetWifiDataWithPasswords*(): seq[WifiData] =
    ...
    for i in 0 ..< interfaceInfoList.dwNumberOfItems:
        ...
        var interfaceName = interfaceInfo.strInterfaceDescription
...
when isMainModule:
    var allWifiData = GetWifiDataWithPasswords()
    for wData in allWifiData:
        echo("Interface: " & wData.InterfaceName)
        echo("SSID: " & wData.Name)
        echo(wData.XmlData) # You can save this to a an xml file.
        echo("----------------")
